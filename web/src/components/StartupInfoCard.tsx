...
export function StartupInfoCard({ startup }: Props): React.ReactElement {
  ...
  const warning = startup.contextWarning;

  return (
    <section className="flex h-full flex-col rounded border border-neutral-200 dark:border-neutral-800">
      {warning && (
        <div className="border-b border-amber-300 bg-amber-50 px-4 py-2 text-xs text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200">
          Context is limited: <span className="font-mono">n_ctx={warning.nCtxSeq}</span> is smaller than trained{' '}
          <span className="font-mono">n_ctx_train={warning.nCtxTrain}</span>. Model capacity won't be fully used.
        </div>
      )}
      <h3 className="px-4 py-3 text-xs font-semibold uppercase tracking-wide opacity-60">Startup</h3>
      <div className="flex-1 border-t border-neutral-200 p-4 dark:border-neutral-800">
